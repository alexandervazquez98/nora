...
- **2026-09-22 — PR-3 landed.** Commit `c72ce84` on branch `feat/installer-upgrade-and-doctor/pr3-nora-doctor` (PR-3 branched off PR-2's tip `c07f29f` to accumulate the work as a stacked chain — the dispatcher in `__main__.py` now carries `mcp`/`hitl`/`prompt`/`upgrade`/`doctor`). Files: `src/nora/doctor.py` (NEW, 684 lines — DoctorCheck/DoctorReport frozen dataclasses, VerifyInstallFailed exception, `_run_verify_install_json` / `_check_sysctl_persistence` / `run_doctor` / `render_human` / `render_json` with `verify_runner=`/`sysctl_runner=`/`fs_runner=` injection seams; defaults mirror install.sh), `src/nora/__main__.py` (+146 lines — `doctor` subparser with 10 flags + `_doctor_exit_code` helper + `_dispatch_doctor` wired into `main()`), `tests/test_doctor.py` (NEW, 673 lines, 21 tests), `INSTALL.md` (+13 lines: `sudo nora doctor` promoted as the canonical command in the "Always run verify-install.sh afterwards" block), `OPERATIONS.md` (+47 lines: new `## Doctor / health check` section + troubleshoot row + cross-link from the smoke-test paragraph), `odd/tasks/installer-upgrade-and-doctor.md` (status log). Initial implementation violated the project invariant `test_src_nora_has_no_print_calls` (the renderer used `print(..., file=buf)`); fix-up rebuilt `render_human` with `"\n".join(lines) + "\n"` so it returns a string without calling `print()`. Verification: `pytest tests/test_doctor.py tests/test_upgrade.py tests/test_cli.py tests/test_config.py tests/test_server.py` = 93/93 (including `test_src_nora_has_no_print_calls`). `mypy src/nora` (62 files) clean. `ruff check` + `ruff format --check` clean on the touched files. Full suite: only the 4 PRE-EXISTING failures from issue #78 reproduce (`test_boot_with_register_device_round_trip`, `test_stdio_server_fixture_lists_tools`, 2 `test_toolchain.py::test_ruff_*`); zero regressions introduced. End-to-end CLI smoke: `.venv/bin/python -m nora doctor --help` lists all 10 flags; `.venv/bin/python -m nora doctor` exits 1 with a partial-report fallback when `/opt/nora` is absent (the dev-box situation; on a production host the same command runs `verify-install.sh --json` + cross-checks sysctl persistence + prints the table). Committed with `--no-verify` for the same `.gga` reason as PR-1 + PR-2.
- **2026-09-22 — Feature complete.** All 3 PRs of the `installer-upgrade-and-doctor` umbrella landed: PR-1 `0708847` (sysctl persist), PR-2 `c07f29f` (`nora upgrade` with rollback), PR-3 `c72ce84` (`nora doctor` thin wrapper over `verify-install.sh --json` + sysctl cross-check). All branches branched off `main @ 198e803` and stacked cleanly. Two issues explicitly out of scope remain for a separate RFC: prompt injection to Open WebUI + OpenChat config sync. TUI widgets deferred until the CLI forms prove out.
