...
5. **`test(perf): investigate -p no:cov -p no:hypothesis for dev loop`** —
   ...
   **Veredicto (medido 2026-09-18)**: NO VALE LA PENA. La diferencia entre
   `addopts` actual (`-q --strict-markers --no-cov -p no:cacheprovider`) y
   la versión plugin-stripped (`... -p no:cov -p no:hypothesis`) es:
   - single-file (`make test-one`): 1.05 s vs 1.04 s (dentro del ruido)
   - suite completa (`make test-fast`): 17.5 s vs 18.0 s (dentro del ruido)

   El cuello de botella real es subprocess boot (4-5 s por test), no
   plugin overhead. Implementar un perfil dev/CI añadiría complejidad
   (env var, conftest hook, o pyproject dual) sin beneficio medible.
   **Acción**: ninguno. Mantener el addopts actual.
...
- **WU-#1a** (`test(perf)`): session-scoped HTTP MCP fixture + 3
  tool-surface tests refactorizados — aplicado en commit `497d0b0`.
  Tests: `test_subprocess_responds_to_tools_list_with_thirteen_tools`,
  `test_subprocess_nora_mcp_exposes_thirteen_tools`,
  `test_subprocess_python_dash_m_nora_exposes_same_tools`. Stdio-specific
  assertions migradas a sus tests dedicados. Setup del fixture (~3.4 s)
  se paga UNA vez por worker; el call de cada test es <0.1 s.

- **WU-#3** (`test(http-smoke)`): session-scoped HTTP fixture para
  `test_http_transport_smoke.py` — aplicado en commit `733722c`. La
  marca `no_xdist` se elimina (puerto libre por worker evita la
  colisión TCP). Los 2 tests ahora corren dentro de `make test-fast`.

- **WU-#5** (`test(perf)`): marcado como **NO VALE LA PENA** —
  documentado en commit `f12095f`. Plugin stripping ahorra ~50 ms
  dentro del ruido. El cuello de botella es subprocess boot, no
  overhead de plugin.

## Resultados finales de la sesión

Comparación antes/después en `chore/test-perf-xdist` (commits
`f6315f9` → `f12095f`):

| Modo                              | Baseline | Final   | Speedup |
| --------------------------------- | -------- | ------- | ------- |
| `make test-fast` (xdist, dev)     | ~23-37 s* | **17.2-18.5 s** | **~1.3-2.1×** |
| `make test` (sequential + cov)    | 105 s    | **42.5 s** | **2.5×** |
| Single-file (`make test-one`)     | 1.4 s    | ~1.05 s | 1.3× |
| Tests corridos en `test-fast`     | 590      | **592** | +2 (http_smoke) |

\* Baseline de `test-fast` fue 23-37 s con flake intermitente por
`test_bootstrap_*` (WU-#4 lo estabilizó a 19 s antes de WU-#1a).

Resumen de commits work-unit (en orden de aplicación):
1. `94a4d38` — WU-0: fix coverage subprocess
2. `2105559` — WU-#4: no_xdist marker para tests flaky
3. `2d8940d` — docs: registrar WU-0/#4 done, WU-#2 no-op
4. `497d0b0` — WU-#1a: HTTP MCP fixture + 3 tests refactorizados
5. `733722c` — WU-#3: HTTP smoke usa el fixture de conftest
6. `f12095f` — WU-#5: docs, no-op (medición)
