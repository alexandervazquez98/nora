...
Materialises ADR #17 (issue #17): multi-root scan (built-in + operator override, override wins) and semver-aware firmware resolution (strict-major hard-fail, minor descending fallback, literal LLM warning). Out of scope: vendor contribution workflow (P3 DEFERRED), `Device` Literal widening (#14), exception rename (#14), `_driver` Protocol rewrite (#14).

## MODIFIED Capabilities

### Capability: oid-catalog

`OidCatalogRegistry.resolve` no longer requires an exact `(vendor, model, firmware)` pin. Two roots are scanned at boot; semver ordering picks the closest valid firmware at resolve time.
...
### Capability: driver-snmp-pmp450i

Wording only: the driver consumes the catalog returned by `OidCatalogRegistry.resolve(...)`, including minor-mismatch fallbacks (see `oid-catalog > Minor Descending Fallback With Literal Warning`). No driver-boundary change.

### Capability: prompt-registry

Cross-reference only: `oid-catalog` mirrors the same layered-load pattern as `Default Source — Package Data` + `Operator Override — NORA_PROMPTS_DIR` (built-in baseline + operator override, override wins). No behavioural change to prompt loading.
